...
Two nn.Module classes:
  ...
  OrganGAT
      ...
      Architecture:
          Linear(NODE_FEAT_DIM → hidden_dim)        # default 6 → 64
          → GATConv(hidden_dim, hidden_dim, num_heads)
          → mean-attention pooling over nodes
          → MLP(hidden_dim × num_heads → 32 → 1)
          → 4 × sigmoid(x) + 1                       # score ∈ (1, 5)

  FaceRankNet
      One OrganGAT per organ (5 total). The forward output is shaped by
      config.FUSION_MODE:
        - "score_aware"  : cross-organ MultiheadAttention → mean-pool →
                           concat(global_embed, local_scores) → MLP → global_score
        - "fusion_weight": global_score = Σ softmax(fusion_weights)[i] *
                           local_scores[i]   (no attention / MLP)
      forward() returns a dict with keys:
          'local_scores'  : dict[str, Tensor]  – one scalar per organ
          'global_score'  : Tensor             – (B,), values in (1, 5)
          'organ_weights' : Tensor | None      – softmax(fusion_weights) when
                                                  mode is fusion_weight, else None
          'attn_weights'  : Tensor | None      – (B, 5, 5) in score_aware mode
          'global_embed'  : Tensor | None      – (B, embed_dim) in score_aware
                                                  mode (for sensitivity diagnostic)
...
class OrganGAT(nn.Module):
    ...
    ----------
    in_feats : int
        Input node feature dimension (default = config.NODE_FEAT_DIM = 6
        for (x, y, z, Δx, Δy, Δz)).
    hidden_dim : int
...
class FaceRankNet(nn.Module):
    ...
    def __init__(self, gat_kwargs: dict | None = None) -> None:
        ...
        if self.fusion_mode not in {"score_aware", "fusion_weight"}:
            raise ValueError(
                f"Unknown FUSION_MODE={self.fusion_mode!r}. "
                "Must be 'score_aware' or 'fusion_weight'."
            )

        embed_dim = config.GAT_HIDDEN_DIM * config.GAT_NUM_HEADS  # 256

        if self.fusion_mode == "fusion_weight":
            # Weighted sum: only learnable fusion_weights, no attention / MLP.
            ...
            # Concat global_embed + local_scores → MLP
            self.global_mlp = nn.Sequential(
                nn.Linear(embed_dim + config.NUM_ORGANS, 64),
                nn.ELU(),
    ...
    def forward(
        ...
        ----------
        subgraphs : dict[str, dgl.DGLGraph]
            Keys must match ``ORGAN_ORDER``. Each value is a (possibly batched)
            DGL graph.

        Returns
        -------
        dict with keys (some may be None depending on fusion_mode):
            'local_scores'  : dict[str, Tensor]  — shape (B,) per organ
            'global_score'  : Tensor             — shape (B,), values in (1, 5)
            'organ_weights' : Tensor | None      — softmax(fusion_weights) when
                                                   fusion_mode == 'fusion_weight'
            'attn_weights'  : Tensor | None      — (B, 5, 5) in score_aware
            'global_embed'  : Tensor | None      — (B, embed_dim) in score_aware
                                                   (for sensitivity diagnostic)
        """
        ...
        if self.fusion_mode == "fusion_weight":
            # Convex combination of local scores → global_score ∈ (1, 5)
            # without needing scale_to_score (weights are a simplex).
            organ_weights = torch.softmax(self.fusion_weights, dim=0)   # (5,)
            global_score = (local_stack * organ_weights).sum(dim=-1)    # (B,)
            return {
        ...
        # score_aware: cross-organ attention + score-aware MLP head
        ...
        mlp_input = torch.cat([global_embed, local_stack], dim=-1)  # (B, embed_dim + 5)
        global_logit = self.global_mlp(mlp_input).squeeze(-1)
        ...
        return {
            ...
            "global_score":  global_score,
            "organ_weights": None,
            "attn_weights":  attn_weights,       # (B, 5, 5)
            "global_embed":  global_embed,       # (B, embed_dim) — for sensitivity diagnostic
        }
